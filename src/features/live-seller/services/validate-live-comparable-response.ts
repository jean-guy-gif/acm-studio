...
export type LiveComparableResponseValidation =
  | { ok: true; value: LiveComparableResponsePatch }
...
export function validateLiveComparableResponse(
  input: LiveComparableResponsePatch,
  ...
  if ('seller_estimated_days_on_market' in input && input.seller_estimated_days_on_market == null) {
    errors.seller_estimated_days_on_market =
      'Indiquez une durée avant de révéler la durée observée.';
  } else if (input.seller_estimated_days_on_market != null) {
    const days = input.seller_estimated_days_on_market;
    if (!Number.isInteger(days) || days < 0 || days > MAX_ESTIMATED_DAYS_ON_MARKET) {
      errors.seller_estimated_days_on_market =
        'La durée doit être un nombre entier de jours positif.';
    }
  }
